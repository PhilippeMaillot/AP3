...
class BetController {
  ...
  static add = async (req, res) => {
    try {
      const { id_tournament, id_club, bet_amount, bet_prediction, id_user } =
        req.body;
  ...
  static update = async (req, res) => {
    try {
      const { id_bet, id_tournament, id_club, bet_amount, bet_prediction } =
        req.body;
  ...
  static processBets = async (req, res) => {
    console.log("Traitement des paris en cours...");
    try {
      await model.processBets();
      res.status(200).json({ message: "Les paris ont bien été traités !" });
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: "Internal Server Error" });
    }
